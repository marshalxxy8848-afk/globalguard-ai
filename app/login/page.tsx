...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error === 'Invalid email or password' ? '邮箱或密码错误' : (data.error || '登录失败'));
      setLoading(false);
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-b from-[#0a0a0f] via-[#0d0d14] to-[#0a0a0f]">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-white/90">欢迎回来</h1>
          <p className="mt-1 text-xs text-white/40">登录 GlobalGuard AI 继续使用</p>
        </div>
